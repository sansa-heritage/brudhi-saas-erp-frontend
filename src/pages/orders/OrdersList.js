...
const Orders = () => {
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      // Try all possible response structures based on your backend
      // Your backend returns: { data: { data: [...] } } or similar
      
      // Structure 1: response.data.data (most common)
      if (response?.data?.data && Array.isArray(response.data.data)) {
        ordersData = response.data.data;
        console.log("Found orders in response.data.data, count:", ordersData.length);
      }
      ...
      // Structure 3: response itself is array
      else if (Array.isArray(response)) {
        ordersData = response;
        console.log("Found orders in response, count:", ordersData.length);
      }
      // Structure 4: response.data.data.data (nested)
      else if (response?.data?.data?.data && Array.isArray(response.data.data.data)) {
        ordersData = response.data.data.data;
        console.log("Found orders in response.data.data.data, count:", ordersData.length);
      }
      // Structure 5: response.data.data.orders
      else if (response?.data?.data?.orders && Array.isArray(response.data.data.orders)) {
        ordersData = response.data.data.orders;
        console.log("Found orders in response.data.data.orders, count:", ordersData.length);
      }
      else {
        console.warn("Could not find orders array. Response structure:", response);
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      ...
      shipped: { bg: "secondary", text: "Shipped" },
  ...
  const getOrderTypeBadge = (type) => {
    const typeConfig = {
      sales: { bg: "success", text: "Sales" },
      purchase: { bg: "info", text: "Purchase" },
      return: { bg: "warning", text: "Return" },
    };
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h2 className="fw-bold mb-1">
            <FaShoppingCart className="me-2 text-primary" /> Order Management
          </h2>
          <p className="text-muted">Manage all customer orders and track status</p>
        </div>
        ...
        <Col md={2}>
          <Card className="border-0 shadow-sm rounded-3">
            <Card.Body className="text-center">
              ...
              <h3 className="fw-bold text-primary mb-0">{stats.processing}</h3>
            </Card.Body>
          </Card>
        </Col>
        <Col md={2}>
          <Card className="border-0 shadow-sm rounded-3">
            <Card.Body className="text-center">
              ...
              <h3 className="fw-bold text-success mb-0">{stats.delivered}</h3>
            </Card.Body>
          </Card>
        </Col>
        <Col md={2}>
          <Card className="border-0 shadow-sm rounded-3">
            <Card.Body className="text-center">
              ...
              <h3 className="fw-bold text-danger mb-0">{stats.cancelled}</h3>
            </Card.Body>
          </Card>
        </Col>
        <Col md={2}>
      ...
      <Card className="border-0 shadow-sm mb-4 rounded-3">
        <Card.Body>
          <Row>
            ...
            <Col md={2}>
              <Form.Select value={orderTypeFilter} onChange={(e) => setOrderTypeFilter(e.target.value)}>
              ...
              <tbody>
                {filteredOrders.length > 0 ? (
                  filteredOrders.map((order) => (
                    <tr key={order.id}>
                      <td>{order.id}</td>
                      <td className="text-primary fw-semibold">{order.order_no}</td>
        ...
        <Modal.Body>
          {selectedOrder && (
            <>
              <Row>
                <Col md={6}>
                  <strong>Order Date:</strong> {formatDate(selectedOrder.order_date)}<br />
                  <strong>Customer:</strong> {selectedOrder.customer_name}<br />
                  <strong>Order Type:</strong> {selectedOrder.order_type}<br />
                  <strong>Status:</strong> {getStatusBadge(selectedOrder.status)}
                </Col>
                <Col md={6}>
                  <strong>Payment Status:</strong> {getPaymentStatusBadge(selectedOrder.payment_status)}<br />
